#!/usr/bin/env python3
"""Подготовка фото врача для сайта «Ангел-Дент».

Единый стиль карточек врачей: портрет вырезается из исходного фона и
ставится на фирменный светло-голубой градиент, кадр — 4:5, голова и плечи,
лицо по центру с небольшим отступом сверху. Так выглядят все врачи
(см. drobkova / rustamov / drobkova) — новые фото обязаны совпадать.
...
Шаги:
  ...
  2. Композит на голубой градиент (верх #EAF2FC → низ #FCFDFE) в кадре 4:5,
...
Примеры:
    python scripts/prepare-doctor-photo.py /tmp/drobkova.jpg drobkova --zoom 0.95
    python scripts/prepare-doctor-photo.py /tmp/rustamov.jpg rustamov
...
# Фирменный светло-голубой градиент фона (как у существующих фото врачей).
BG_TOP = (234, 242, 252)   # #EAF2FC
BG_BOT = (252, 253, 254)   # #FCFDFE
...
def compose(cut: Image.Image, zoom: float, top_frac: float) -> Image.Image:
    """Силуэт на голубой градиент, кадр 4:5, лицо по центру у верха."""
...
def main(argv: list[str]) -> int:
    p = argparse.ArgumentParser(description="Фото врача → голубой градиент 4:5")
