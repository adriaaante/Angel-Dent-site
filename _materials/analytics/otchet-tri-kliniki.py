...
SEARCH = [
    #  клиника,      показы было, стало, страниц в поиске, ИКС
    ("Ангел-Дент",   5401,  9152, 42, 30),
    ("Версаль",       663,  4265, 50, 10),
    ("Венеция",         0,    23, 31,  0),
]

# Метрика: визиты, посетители, звонки (клик по номеру), мессенджеры, заявки.
# Версаль и Венеция — посещаемость и обращения (Метрика).
GROWTH = [
    #  клиника,   визиты было, стало, звонки было, стало, мессенджеры, заявки
    ("Версаль",   1019, 2587, 16, 37, 1, 31, 6, 10),
    ("Венеция",      0,   94,  0,  0, 0,  2, 0,  2),
]

# Ангел-Дент — видимость и мессенджеры.
ANGEL = [
    ("Показы в поиске Яндекса", 5401, 9152),
    ("Обращения в WhatsApp и Telegram", 13, 30),
    ("Обращения в Telegram", 3, 14),
]
...
Q_ANGEL = [
    ...
    ("протезирование зубов в реутово",   62, 263, 9.1, 8.7),
    ("детская стоматология в реутове",    0,  77, 0.0, 7.3),
]
...
def search_table(s) -> Table:
    rows = []
    for name, sh0, sh1, pages, sqi in SEARCH:
        rows.append([
            clinic_cell(name, s),
    ...
    tot = [sum(r[i] for r in SEARCH) for i in (1, 2, 3)]
    rows.append([
        Paragraph("<b>Три клиники вместе</b>", s["cellb"]),
        Paragraph(f"{num(tot[0])} → <b>{num(tot[1])}</b>", s["cellb"]),
        grow_cell(tot[0], tot[1], s),
        Paragraph(f"<b>{tot[2]}</b>", s["cellb"]),
        Paragraph("—", s["cellm"]),
    ])
    return table(s, ["Клиника", "Показы в выдаче", "Рост", "Страниц в поиске", "ИКС"],
                 rows, [34 * mm, 40 * mm, 24 * mm, 34 * mm, 20 * mm],
                 [("BACKGROUND", (0, len(rows)), (-1, len(rows)), SOFT)])


def growth_table(s) -> Table:
    rows = []
    for name, v0, v1, c0, c1, m0, m1, l0, l1 in GROWTH:
        rows.append([
            clinic_cell(name, s),
            Paragraph(f"{num(v0)} → <b>{num(v1)}</b>", s["cell"]),
            grow_cell(v0, v1, s),
            Paragraph(f"{c0} → <b>{c1}</b>" if c1 else "—", s["cell"]),
    ...
    return table(s, ["Клиника", "Визиты", "Рост", "Звонки с сайта",
                 ...
                 rows, [27 * mm, 31 * mm, 19 * mm, 27 * mm, 33 * mm, 31 * mm])


def angel_table(s) -> Table:
    rows = [[Paragraph(label, s["cell"]),
             Paragraph(f"{num(a)} → <b>{num(b)}</b>", s["cell"]),
             grow_cell(a, b, s)] for label, a, b in ANGEL]
    return table(s, ["Показатель", "Было → стало", "Рост"],
                 rows, [78 * mm, 50 * mm, 25 * mm])
...
def build() -> None:
    ...
    story = [
        ...
        P("Сильнее всех выросла <b>«Версаль»</b>. В июне её в поиске практически "
          ...
          "звонков с сайта <b>16 → 37</b>, обращений в мессенджеры <b>1 → 31</b>, "
          "заявок с формы <b>6 → 10</b>.", "p"),
        P("<b>«Ангел-Дент»</b> прибавил там, где закладывается будущий поток: "
          "показов в выдаче стало на <b>69 % больше</b>, позиции по дорогим "
          "коммерческим запросам поднялись, а обращений в мессенджеры стало "
          "<b>13 → 30</b>, в том числе в Telegram — в четыре с лишним раза.", "p"),
        P("<b>«Венеция»</b> вышла в поиск с нуля: 31 страница в индексе, первые "
          ...
          "люди приходят сами, из поиска, и читают клинику вдумчиво.", "p"),
        ...
        P("«Показы» — сколько раз страницы сайта показались людям в выдаче "
          ...
          "сайтов он растёт медленно, и у «Версаля» он уже сдвинулся с нуля. "
          "123 страницы трёх сайтов в индексе — это 123 входа, по которым "
          "клинику может найти пациент.", "small"),
        ...
        P("Шесть из семи запросов в этой таблице раньше не приносили сайту "
          "вообще ничего. Сейчас по ним «Версаль» на первой странице выдачи, "
          "а переходов из поиска стало вдвое больше (35 → 73).", "small"),

        P("«Ангел-Дент»: позиции подросли на дорогих запросах", "h2"),
        ...
        P("Имплантация поднялась с 8,7 на 7,0, протезирование с 8,7 на 8,1, "
          ...
          "своя страница: до этого тема была спрятана внутри страницы гигиены. "
          "Это и есть прямой эффект работы по сайту.", "p"),
        Spacer(1, 4),
        angel_table(s),

        P("«Версаль» и «Венеция»: люди и обращения", "h2"),
        growth_table(s),
        Spacer(1, 4),
        ...
        P("«Версаль» набирает аудиторию месяц к месяцу (визиты)", "h2"),
        months_table(s),
        Spacer(1, 4),
        P("У «Версаля» половина сентября уже перекрыла весь июль и идёт выше "
          "августа — сайт набирает аудиторию сам, из поиска. «Венеция» "
          "стартовала в августе и с первого месяца получает визиты и заявки.", "p"),
        ...
        P("31 страница в индексе, первые показы и восемь переходов из поиска, "
          ...
          "хороший старт: у молодого домена первые месяцы уходят на накопление "
          "доверия поисковика, и «Венеция» этот этап уже прошла — дальше "
          "количество запросов, по которым её показывают, будет расти так же, "
          "как выросло у «Версаля».", "p"),
        Spacer(1, 4),
        P("Следующий шаг для неё — <b>отзывы</b>: табличка с QR-кодом на "
          "карточку Яндекса уже сделана и лежит в материалах, её остаётся "
          "распечатать и поставить на стойку. Каждая новая карточка с отзывами "
          "поднимает клинику и в Картах, и в поиске.", "p"),
        ...
        P("Что дальше — и что это даст", "h2"),
        P("<b>1. Из 7–9 позиции в топ-5.</b> Мы уже показываемся по всем "
          "ключевым запросам города — теперь задача поднять «Ангел-Дент» и "
          "«Версаль» выше. В топ-5 кликают в разы чаще, и накопленные показы "
          "начнут превращаться в звонки.", "p"),
        P("<b>2. Отзывы для «Версаля» и «Венеции».</b> Таблички с QR готовы; "
          "нужно поставить их на стойку и просить отзыв после приёма — это "
          "самый быстрый рычаг для Карт.", "p"),
        P("<b>3. Технические настройки «Венеции».</b> Привяжем счётчик Метрики "
          "к Вебмастеру и снимем старую служебную отметку домена (она осталась "
          "от прежнего владельца адреса) — Яндекс станет охотнее показывать "
          "молодой сайт.", "p"),
        P("<b>4. Публикации в Яндекс.Бизнесе раз в месяц</b> по каждой клинике: "
          "это напрямую влияет на место карточки в Картах. Материалы собраны, "
          "нужна только заливка.", "p"),
        P("<b>5. Продолжаем наращивать страницы и документы врачей</b> — именно "
          "они за эти полтора месяца дали рост показов и позиций.", "p"),
        ...
        P("Посещаемость, звонки, мессенджеры и заявки — Яндекс.Метрика "
          ...
          "отправку формы. Показы, позиции, страницы в поиске и ИКС — "
          "Яндекс.Вебмастер по трём подтверждённым хостам. Периоды равной "
          f"длины: {PERIOD} (46 дней) против {PREV} (46 дней). Ничего не "
          "округляли и не досчитывали: все цифры в отчёте — выгрузка как есть.", "small"),
        Spacer(1, 8),
        signature(s),
    ]
    doc.build(story)
