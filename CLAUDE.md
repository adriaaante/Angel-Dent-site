...
Файл и генератор — в `_materials/yandex-business/` (не деплоится, `_materials`
исключена). **Источник цен — САЙТ** (`services/*.html`, `promotions.html`):
сначала правим цену/акцию на сайте, потом пересобираем файл
`python3 _materials/yandex-business/build-price.py`. НЕ наоборот. Правила
оформления (цена не дублируется в названии, «от/вместо» — в описании,
бесплатное = 0, без просроченных дат, у брекетов без «1 челюсть») — в
