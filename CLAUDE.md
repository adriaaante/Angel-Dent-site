...
Картинки фида: **позиционные** (согласованные кадры витрины, 16.07.2026) в
`assets/img/yb/pos/yb-pos-*.jpg` — маппинг `POS_IMG` в генераторе по точному
названию позиции; для остальных позиций — категорийные `assets/img/yb/yb-*.jpg`
(всё деплоится). В фид идут только позиции с ценой; бесплатные/подарочные
акции — в раздел «Акции» карточки YB. Витрину Карт заполнять «Позициями из
Товаров и услуг» (картинки уже правильные); нюанс: это копирование на момент
нажатия, не живая связь — после больших правок фида пересобрать витрину.
...
Картинка акции = **мини-баннер**, а не «просто лицо»: формат 1200×1200,
бренд-шапка (лого + «АНГЕЛ-ДЕНТ / Стоматология · Реутов»), жёлтый бейдж
(«−44%» / «Подарок» / «0 ₽»), **выгода крупно цифрой** (новая цена +
зачёркнутая старая), фото справа с растушёвкой в фон, мелкий дисклеймер
«Имеются противопоказания…» на каждой. Скрипт —
`_materials/yb-promo/promo-cards.py` (фото-исходники — Higgsfield soul_2,
можно переиспользовать фото из сторис; правила людей те же: сгенерированные,
славянская внешность, привлекательные, без до/после, фейковые надписи
зачищать).
...
- Меняется акция на сайте → перегенерировать карточку и обновить google-doc
  «Акции ЯБ — карточки и инструкция» (папка Drive «Акции ЯБ — карточки»).
